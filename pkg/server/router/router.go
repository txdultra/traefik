...
func (m *Manager) buildHTTPHandler(ctx context.Context, router *runtime.RouterInfo, routerName string) (http.Handler, error) {
	...
	return chain.Extend(*mHandler).Append(tHandler).Append(lastHandlers...).Then(sHandler)
}
...
var (
	lastHandlers []alice.Constructor
)

func AppendLastHandler(next func(http.Handler) (http.Handler, error)) {
	lastHandlers = append(lastHandlers, next)
}
