...
run-dev:
	...
	./traefik --api.dashboard=true --api.insecure=true --api.debug=true --providers.etcd=true \
		--providers.etcd.rootKey=cdf --entryPoints.web.address=:83 --entryPoints.websecure.address=:448  \
		--log.level=DEBUG

.PHONY: build-dist
build-dist:
	#go generate
	go build -o dist/ ./cmd/traefik
	docker build -t cdf-gateway:v1.0 . --load